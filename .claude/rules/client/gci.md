...
`client/src/gciLibrary/optionalFunctions.ts` is the single source of truth for which GCI bindings are optional and why (`addedIn`/`absentOn`/`removedIn`). Binding a symbol with `optionalFunc` requires an entry there — `_optional` in `gciLibrary.ts` is a mapped type over the registry's keys, so a missing or stale entry is a compiler error. Most of it isn't written by hand: the version- and platform-gated entries are generated from the parsed headers in `vendor/gci-headers/` into the committed `optionalFunctions.generated.ts` (`npm run generate:gci-optional-functions`, rerun by CI, which fails on any diff), and `optionalFunctions.ts` spreads that and adds only `removedIn` — the one claim no vendored snapshot can make. So after vendoring a header revision, regenerate; don't edit the generated file.

Every symbol in that registry is gated out of production code by `eslint.config.mjs`'s `OPTIONAL_GCI_CALL` block, which generates a `no-restricted-syntax` selector per entry — the config imports `optionalFunctions.ts` directly (Node strips the types), so the gated set is never restated. Keep that file free of non-erasable TypeScript (`enum`, `namespace`) or lint breaks while compile still passes. A version- or platform-conditional fallback belongs **inside** `client/src/gciLibrary/`: optionality is the binding layer's knowledge, so code above it calls a helper rather than pairing `isAvailable`/`supportsNonBlockingLogin` with a raw optional binding. The three call sites that predate this rule carry an `eslint-disable-next-line` naming their guard and degraded path; prefer the helper over a fourth.
