...
`client/src/gciLibrary/optionalFunctions.ts` is the single source of truth for which GCI bindings are optional and why (`addedIn`/`absentOn`/`removedIn`). Binding a symbol with `optionalFunc` requires an entry there — `_optional` in `gciLibrary.ts` is a mapped type over the registry's keys, so a missing or stale entry is a compiler error. Most of it isn't written by hand: the version- and platform-gated entries are generated from the parsed headers in `vendor/gci-headers/` into the committed `optionalFunctions.generated.ts`, and `optionalFunctions.ts` spreads that and adds whatever claim no vendored snapshot can make.
