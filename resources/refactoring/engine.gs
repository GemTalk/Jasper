...
removeallmethods GsRefactoringUndo
removeallclassmethods GsRefactoringUndo
...
removeallmethods GsSplitClassRefactoring
removeallclassmethods GsSplitClassRefactoring
...
category: 'private'
method: GsRefactoringUndo
setLabel: aLabel engine: anEngineName sequence: aSequence reverseOp: anArray
	label := aLabel isNil ifTrue: [nil] ifFalse: [aLabel asString].
	engineName := anEngineName isNil ifTrue: [nil] ifFalse: [anEngineName asString].
	sequence := aSequence.
	reverseOp := anArray
...
category: 'private'
method: GsRefactoringUndo
setLabel: aLabel engine: anEngineName sequence: aSequence changeSet: aChangeSet
	label := aLabel isNil ifTrue: [nil] ifFalse: [aLabel asString].
	engineName := anEngineName isNil ifTrue: [nil] ifFalse: [anEngineName asString].
	sequence := aSequence.
...
category: 'recording'
classmethod: GsRefactoringUndo
recordAndApplyForToken: token engine: engineName deselected: deselectedIds label: aLabel
...
category: 'recording'
classmethod: GsRefactoringUndo
result: aJsonString withUndoRecorded: aBoolean
...
category: 'recording'
classmethod: GsRefactoringUndo
entryFrom: beforeStates to: afterStates label: aLabel engine: engineName
...
category: 'recording'
classmethod: GsRefactoringUndo
stageRestore: aState in: aChangeSet
...
category: 'recording'
classmethod: GsRefactoringUndo
stageRecompile: beforeState over: afterState in: aChangeSet
...
category: 'recording'
classmethod: GsRefactoringUndo
stageRemove: afterState in: aChangeSet
...
category: 'private'
method: GsRenameClassRefactoring
superclassForShapeSource: src of: old
	"The superclass the new version belongs under.

	 For an ordinary RENAME src is the current class, so this is just its current superclass and
	 nothing changes. For a RESTORE src is a historical version, and its PARENTAGE is part of the
	 definition being restored -- so a restore must go back under the parent that version had, not
	 stay under whatever parent the class has now. Passing `old superclass` here meant a restore
	 silently kept the current parent, so restoring across a re-parenting (an inserted superclass,
	 an extracted one) claimed to restore a definition it did not restore. Undoing such a
...
applyClassRename: aChange
	...
	new := self
		makeNewVersionOf: old
		shapedLike: src
		named: aChange newName
		superclass: (self superclassForShapeSource: src of: old).
...
setEnvironment: anEnvironment class: aSource name: aName dict: aDictOrNil extractIvars: ivars
	environment := anEnvironment.
	sourceClass := aSource.
	newName := aName asString.
	dictNameOrNil := aDictOrNil isNil ifTrue: [nil] ifFalse: [aDictOrNil asString].
...
category: 'private'
method: GsSplitClassRefactoring
selectorText: aSelector args: argNames
...
category: 'instance creation'
classmethod: GsSplitClassRefactoring
class: aSource splitIntoClassNamed: aName extractingInstVars: ivars inDictionary: aDictOrNil
