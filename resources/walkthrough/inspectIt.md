...
behind it. Double-click any row to open that object in a new column to the right, so
the trail you followed stays on screen. The **Evaluate** tab runs an expression with
the inspected object bound to `self`, on the same `Cmd/Ctrl + K` keys you use in an
editor — `D` to display the result, `E` to execute, `I` to inspect it.
