...
- **GemStone Manager: one screen for the whole GemStone environment.** A new **GemStone Admin: GemStone Manager** editor tab shows the operating system's prerequisites, the versions installed, the databases made from them, and the logins that reach those databases — in the order they matter, so a machine that cannot hand out enough shared memory says so before anything else, a machine with nothing installed leads with the versions, and once neither is true the screen leads with connecting. It hosts every action the administrative sidebar views offered, each dispatched to the command that already owns it — and those views leave the sidebar for it (see **Changed**). The operating-system section is a checklist with one row per prerequisite the machine actually has — shared memory (with how much is still free, not just the limit), RemoveIPC, the WSL version, mirrored networking, the two `gs64ldi` services entries, the hosts entry — each carrying its own remedy only while it is failing. It keeps itself current as stones start and stop, logins are added, and sessions come and go. ([#454](https://github.com/GemTalk/Jasper/pull/454))
- **GemStone Manager: see (and change) a session's configuration.** Once a session is selected, the Manager grows a **Configuration** section that reads the stone and gem configuration reports over that session and lists every parameter, alphabetized, with its current value, in two collapsible groups — Stone and Session (Gem) — so one can be folded away to reach the other. The two naming families the reports mix together are drawn apart: an ALL_CAPS config-file parameter (set before startup) is marked **read-only**, and a CamelCase runtime parameter is marked **runtime** and carries an inline editor — a true/false chooser for a Boolean, a text field otherwise. Each row carries an info tooltip stating the value's type and settability, plus the parameter's purpose where the product's `system.conf` explains it — and where it does not, the tooltip says why (this runtime parameter's config-file name did not match, or no `system.conf` was found for the version) rather than leaving a blank. A filter box narrows the list by name. Editing is offered only where it can succeed: stone parameters can be changed only by SystemUser, so for any other user they read as runtime-but-not-editable with the reason in the tooltip, rather than presenting an editor the stone would reject. Beyond that the stone is the authority: a change it refuses — a gem parameter frozen after login, say — comes back with the stone's own words, and a change it accepts but does not actually apply (a value fixed at startup) is reported by showing the settled value rather than passing silently. The values load only when the section is opened, so an environment change never pays for a round-trip to the gem. ([#232](https://github.com/GemTalk/Jasper/issues/232))
- **GemStone Manager: a guided path from nothing to a running database.** For a machine with nothing set up yet, the Manager leads with what to do next — a header line names the next step and how many of the four are done, a **Show me how** coach-mark tour points at each control in turn, and **Quick Setup** is surfaced up front rather than left to be found. Two zero-question commands back it: **Create Database (defaults)** builds a database on the newest installed version with default names, adds its DataCurator login, and starts the stone and NetLDI — stopping with the stone's own message if a start fails — and **Create Login** adds the login a managed database implies and logs straight in. A **Refresh** re-reads the machine and the download catalogue on demand. ([#454](https://github.com/GemTalk/Jasper/pull/454))

### Changed

- **The GemStone views collapse into a single activity-bar icon.** The separate second container is gone: the Explorer's panes — Dictionaries, Class Categories, Classes, Hierarchy, Methods, and Open Editors — now share the one **GemStone Explorer** activity-bar container with Rowan, the Inspector, and the MCP Server view. The administrative views — Versions, Databases, Processes, Logins & Sessions, and Configure OS — leave the sidebar; the GemStone Manager is where they are managed now. ([#454](https://github.com/GemTalk/Jasper/pull/454))

### Fixed

- **Three environment-management fixes.** A login row that still offered **Log in** after its session had connected now reflects the live session; deleting a locally-created database also removes the local login Jasper made for it — a remote login that merely shares the default stone name is left untouched — rather than leaving an entry that can only fail; and the extent chooser no longer offers to change an extent while the stone is running, which the replace refuses. ([#454](https://github.com/GemTalk/Jasper/pull/454))
