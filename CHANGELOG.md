...
- **The Methods pane's Filter button opens VS Code's own find box, inside the pane.** Filtering used to mean a floating input box at the top of the window, which had to guess what a click somewhere else meant — the reason clicking a result could select the wrong row. VS Code's find box opens in the pane itself, at its top right: type to narrow, press its funnel toggle to hide the rows that don't match rather than merely highlight them (the toggle stays on for that pane once set), click a result without losing what you typed, and Escape or its ✕ to dismiss. Filtering the Methods pane to the methods that read, write or reference an instance variable is unchanged — it still comes from the instance variable's context menu, and the find box searches within the rows it selected. The other three panes keep the old box for now. ([#518](https://github.com/GemTalk/Jasper/issues/518), [#523](https://github.com/GemTalk/Jasper/issues/523))
...
- **Clicking a row in a filtered Explorer pane now selects the row you clicked.** With a filter active and its input box still open, the click that landed on a result also dismissed the box — and a dismissal counted as a cancel, so the filter was undone and the full list snapped back under the pointer before the click resolved, selecting whichever row had moved into that spot. Filtering a long list is exactly when you most need to click a result, so the filter was unusable for its main purpose. The box now stays open when focus leaves it, and nothing moves: clicking a result keeps the filter and puts the box away. Escape still cancels an edit and restores the filter that was in force when the box opened, and the filter row's ✕ still clears an accepted filter. The Dictionaries, Class Categories and Classes panes share the one code path, so all three are fixed together; the Methods pane now narrows with VS Code's own find box instead (see Changed). ([#518](https://github.com/GemTalk/Jasper/issues/518))
