...
- **Undo.** Editing GemStone code is no longer one-way. One **Undo** covers everything you have done this session — saving a method, adding one, deleting one, and applying a refactoring — most recent first. A purple **↩ Undo** button sits in the **status bar** whenever there is something to undo, and its tooltip names the specific thing it would reverse; the same action is in the Explorer's Dictionaries title bar beside Commit and Abort, in the Command Palette, and on the Undo button of the toast that follows a refactoring. Nothing commits in either direction, so undoing uncommitted work stays uncommitted and undoing committed work needs your own commit. Undo is per session: a reconnect starts empty, and an **abort** clears it, since an abort has already rewound the stone underneath every entry. ([#434](https://github.com/GemTalk/Jasper/issues/434))
