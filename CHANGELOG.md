...
- **Renames and instance-variable add/remove can be put back too — by applying the opposite operation.** Renaming a class, an instance variable or a class variable, and adding or removing an instance variable, are now reversible from the same Undo affordances. It is deliberately **not** a rollback, and the preview says so: GemStone has no transaction savepoints, so reversing a rename is a fresh rename in the other direction, and the class keeps its history rather than losing a version. That also makes it *safer* than a true rollback here — anything you wrote after the rename is carried forward instead of discarded. If the old name has since been taken by something else, the reversal refuses up front and names the collision rather than clobbering it. Every remaining class reshape — instance-variable push up/down, convert temporary, extract superclass, insert superclass, split class — is reversible too, by returning each class the refactoring reshaped to the version it had **before** it (shape *and* methods) and unbinding any class the refactoring created. That genuinely restores the pre-refactoring state, which means anything written on those classes since is discarded — so the preview says so up front, names how many methods, and names them per row. Reversing an *added* instance variable removes it again, which deletes any method written since that uses it — the banner names the number; reversing a *removed* one declares the name again but cannot bring back the values it held or the methods the removal dropped, and says so. An apply that **migrated instances** records no reversal at all, because that moved user data. An apply that **migrated instances** or **deleted old versions from the class history** records no undo at all: both commit, and both are irreversible. ([#434](https://github.com/GemTalk/Jasper/issues/434))
