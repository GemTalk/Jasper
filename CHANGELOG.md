...
- **Revealing a class no longer hides the rest of the dictionary.** Jumping to a class — after a rename or reshape, from GemStone Search, from Find Class — pinned the Class Categories pane to that class's own category, which filtered the Classes pane down to it. On a category holding one class, the whole dictionary appeared to vanish behind the class you had just renamed. A reveal now leaves the category selection alone unless keeping it would hide the class, so you get the dictionary's class list with the revealed class selected. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Class Definition History → Restore now restores the class's superclass too.** Restoring a version rebuilt the class with its historical shape and methods but left it under whatever superclass it has *now*, so restoring across a re-parenting (an inserted or extracted superclass) silently did not restore the definition it claimed to. Found while building refactoring undo on top of it. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **A class comment save that the stone refused no longer reports success.** `setClassComment` reports a class it cannot resolve by *returning* a status string rather than raising, and both the Comment panel and the `gemstone://…/comment` editor ignored it — so "Comment updated for …" appeared over a save that wrote nothing. Both now surface what the stone said. Found while making the comment save undoable. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Dragging a method onto a category you just created no longer fails.** The Explorer's **+ new category** button deliberately leaves the stone untouched until something is filed there, so dropping a method on one of those rows targeted a category GemStone did not have — and answered *"the category name '…' was not found"* over a row the pane was showing. A drop now creates the category first, exactly as compiling a method into it already did. ([#434](https://github.com/GemTalk/Jasper/issues/434))

### Added

- **Undo.** Editing GemStone code is no longer one-way. One **Undo** covers everything you have done this session — saving a method, adding one, deleting one, moving one to another category, creating a class, changing its definition, removing it, saving a class comment, adding a class variable, adding accessors, creating or creating or renaming a method category, restoring a class version, moving a class between dictionaries or class categories, creating, renaming or removing a dictionary, and applying a refactoring — most recent first. The affordance you will actually use is the **Undo button on the notice that follows the action**: all of those end in a toast that offers to put it back. For when that is missed, a **↩ Undo** button sits at the left end of the **status bar** for as long as you are connected: purple when there is something to undo and dimmed when there is not, but always in the same place, so it can be found a second time. Hovering it names the exact change it would reverse — **GemStone — Undo: Save Account>>#balance (Ctrl+K U)** — and the verb tells you which kind it is: **Revert** for a class edit, **Undo** for everything else. There is also **Ctrl+K U**, an ↩ icon on the Explorer's **Methods** pane and on the editor title bar, and a **Command Palette** entry — each of which reads **Undo Last Change… (the status bar names it)** or **Revert Last Class Change… (the status bar names it)** — the verb matches, and the hint says where to look, because a contributed menu title is fixed text and cannot name the change itself. Nothing commits in either direction, so undoing uncommitted work stays uncommitted and undoing committed work needs your own commit. Undo is per session: a reconnect starts empty, and an **abort** clears it, since an abort has already rewound the stone underneath every entry. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Undoing an ordinary method edit needs nothing installed on the stone, and asks nothing first.** Saving a method, creating one and deleting one are reversed on the spot — no preview panel, because you just did it and it is one method — putting back the earlier source *and* its category, reloading any open editor and selecting the method that came back. The last 25 edits stay reversible, so you can walk several saves back. The one thing that stops and asks is drift: if the method has changed since that edit, undoing discards the change, and you are told before it happens rather than after — and it is still a warning, never a refusal. This path is plain Smalltalk against the kernel, so it works on any stone Jasper can log in to, whether or not the refactoring engine is installed. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Class actions can be put back too — as a revert.** Creating a class, changing its definition (adding or removing an instance variable, re-parenting it, changing its class variables) and removing it now go on the same stack, reached by the same button and the same **Ctrl+K U**. The word is *revert* rather than undo, and the difference is real: GemStone has no transaction savepoints and gives a class a new version on every shape change, so what happens is that the **earlier version is bound again** — the class history grows rather than shrinks, and anything written on the newer version since is left behind on it. You are told that before it happens, in a modal that **names the methods** it would leave, rather than after. Removing a class is the one case that is exact: the name is unbound but the class itself is not destroyed, so the very same version goes back with its methods, its class-side methods, its history and its instances. Removing a class **with subclasses** from the Explorer records the whole subtree as one entry, and one revert puts all of it back. Reverting a definition change is also the quickest way back from a GemStone behaviour worth knowing about: re-sending `Object subclass: …` with a changed shape answers a **new, empty version** — it does not carry the methods forward — which is why the refactoring engine has its own Add/Remove Instance Variable. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Undo a refactoring.** A refactoring keeps its preview, because it can rewrite dozens of methods across a hierarchy. Undo covers the refactorings that change methods — change signature, extract/inline method, extract/inline temporary, move method, push up, push down, rename method, rename temporary — and a missed toast does not strand it. The preview is the one the forward direction gets: every reversal listed with its diff and its own checkbox, so you can keep part of an undo, and the rows badged with what undoing *does* (Restore / Revert / Delete). A method edited since the refactoring is flagged inline and summarised at the top, and the undo still runs: drift is a warning, never a refusal. After an undo that restored a method, the Explorer selects that method, so what came back is where you are looking. The stone keeps the most recent refactoring only, so applying a second one replaces it. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Every class reshape can be put back too, by returning it to its pre-refactoring state.** Instance-variable push up / push down, convert temporary to instance variable, extract superclass, insert superclass and split class are now reversible: each class the refactoring reshaped goes back to the version it had **before** it — shape *and* methods — and any class the refactoring created is unbound. Because a revert restores the methods of that version too, anything written on those classes since the refactoring is discarded, so the preview says so up front, names how many methods, and names them per row; when nothing has been written since, it says that instead. It is all-or-nothing (each subclass is restored onto its parent), so the rows are shown but not de-selectable. An apply that **migrated instances** or **deleted old versions from the class history** records no reversal at all — both commit, and both are irreversible. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **Renames and instance-variable add/remove can be put back too — by applying the opposite operation.** Renaming a class, an instance variable or a class variable, and adding or removing an instance variable, are now reversible from the same Undo affordances. It is deliberately **not** a rollback, and the preview says so: GemStone has no transaction savepoints, so reversing a rename is a fresh rename in the other direction, and the class keeps its history rather than losing a version. That also makes it *safer* than a true rollback here — anything you wrote after the rename is carried forward instead of discarded. If the old name has since been taken by something else, the reversal refuses up front and names the collision rather than clobbering it. Reversing an *added* instance variable removes it again, which deletes any method written since that uses it — the banner names the number; reversing a *removed* one declares the name again but cannot bring back the values it held or the methods the removal dropped, and says so. An apply that **migrated instances** or **deleted old versions from the class history** records no reversal at all: both commit, and both are irreversible. ([#434](https://github.com/GemTalk/Jasper/issues/434))
- **A class comment and an added class variable can be undone too.** Two more everyday actions join the same **Undo** button, the same **Ctrl+K U** and the same toast. Saving a **class comment** — from the Comment panel or from a `gemstone://…/comment` editor — puts the earlier text back, and it is an exact *undo* rather than a revert: `comment:` does not give the class a new version, so nothing is left behind on an older one. A class that had no comment is a comment of the empty string as far as GemStone is concerned, so undoing a first comment empties it again. Adding a **class variable** is reversed by removing it, and the accessors the same action generated go with it — one action, one Undo, rather than a variable removed while two methods still read a binding the class no longer declares. An accessor you had already written by hand is left exactly alone, because the add skipped it. Anything else that came to reference the variable — a later **Add Accessors** run, a subclass method, one you wrote yourself — is **named in a modal before the undo runs**, because undoing does *not* remove those and GemStone does not break them loudly: it severs the reference, so each one stays in place, reads nil from then on, and no longer recompiles. The scan covers both sides and the whole subtree, which is the variable's actual visibility. Neither action reshapes the class, so there is no version to bind back and no discard modal, and drift — as everywhere else — is a warning rather than a refusal. ([#434](https://github.com/GemTalk/Jasper/issues/434), [#452](https://github.com/GemTalk/Jasper/issues/452))
- **Five more actions can be undone: accessors, method categories, class versions and dictionaries.** **Add Accessors** on its own now records what it compiled — and only what it compiled, so an accessor you had already written by hand is left alone; run as the follow-up to Add Class Variable it still rides on that one entry, so one action stays one Undo. Renaming a **method category** is undone by renaming it back, not by recompiling the methods in it: GemStone refuses to rename one category onto another, which is what makes it exact, and if the old name has been taken again the undo says which name is in the way rather than letting the stone's error reach you unexplained. **Moving a method to another category** — by dragging it onto the row, or from the System Browser's **Move to Category…** — is undoable as well, and a drag that moved several is one Undo, because it was one drag. **Creating** a category with the **+** button, and renaming a **still-empty** one with the pencil, are undoable too, and look no different from the rest: they live only in the Explorer's pane until something is filed there, so the reversal happens there — and if you file a method into one in the meantime, the undo notices and acts on the stone instead. Undoing a *create* whose category has since been filled **refuses**, and says how many methods are in it: GemStone's `removeCategory:` deletes the methods in a category along with it rather than refusing, so that is never attempted on your behalf. Restoring a **class version** from the history viewer is an ordinary class edit and reverts the same way — including across a rename, where it records both names, so reverting rebinds the one the class had and unbinds the one the restore gave it. And the **symbol list** is covered at last: a **created** dictionary is taken back off the list, a renamed one is renamed back, and a **removed** one goes back *at the position it held* — a symbol list is ordered and name resolution walks it in order, so putting it back on the end would silently change what a bare name resolves to. It comes back with every class it held, because removing a dictionary only unlists it. That applies to both ways of removing one: the Explorer's **Remove Dictionary** and deleting a dictionary directory from the file-in mirror. Undoing a *create* asks first if the dictionary has been filled since, and says how many entries are in it: unlisting it puts all of them out of reach, though nothing is deleted — a dictionary is unlisted, never destroyed. Those two used to report themselves in the status bar; they now use a notice, because that is what can carry the Undo button. ([#434](https://github.com/GemTalk/Jasper/issues/434), [#452](https://github.com/GemTalk/Jasper/issues/452))
- **Moving a class is undoable, and both ways of moving it are now in the Explorer.** **Move to Dictionary…** and **Move to Category…** are on the class context menu in the **Classes** and **Hierarchy** panes — they existed only in the classic System Browser before. Moving to another dictionary rebinds the same class object, so nothing is re-versioned and no method moves; undo binds it back where it came from. Filing a class under another **class category** changes a label on the class, so nothing is recompiled. A category rename is recorded **per class**, which is what makes it reversible when the rename **merges** into a category that already exists: putting the name back would drag along the classes that were already there, while putting back each class's own former label is exact — and the same holds for a rename that moved a dash-segmented subtree or skipped a class it could not write. ([#434](https://github.com/GemTalk/Jasper/issues/434), [#452](https://github.com/GemTalk/Jasper/issues/452))
