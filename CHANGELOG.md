...
- **GemStone Manager: see (and change) a session's configuration.** Once a session is selected, the Manager grows a **Configuration** section that reads the stone and gem configuration reports over that session and lists every parameter with its current value. The two naming families the reports mix together are drawn apart: an ALL_CAPS config-file parameter (set before startup) is marked **read-only**, and a CamelCase runtime parameter is marked **runtime** and carries an inline editor — a true/false chooser for a Boolean, a text field otherwise. A filter box narrows the list by name, and where the product's `system.conf` explains a parameter, that explanation rides along as the parameter's tooltip. Editing lets the stone be the authority: a change it refuses — a stone parameter that needs SystemUser, a gem parameter frozen after login — comes back with the stone's own words rather than a silent no-op. The values load only when the section is opened, so an environment change never pays for a round-trip to the gem. ([#232](https://github.com/GemTalk/Jasper/issues/232))
