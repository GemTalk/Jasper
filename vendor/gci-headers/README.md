...
Vendor snapshots of the GCI (GemStone C Interface) header files — `gci.ht`, `gcicmn.ht`, `gcits.hf`, and (from 3.7.2 onward) `gcits.ht` — copied verbatim from a GemStone server distribution's `include/` directory. They are the authoritative reference for GCI function signatures, struct layouts, and constants; not authored by this project, so don't edit them.
