/**
 * Pure HTML rendering for the Inline Temporary (M4) preview panel. M4 is
 * method-local, so there is exactly ONE change: the method recompiled with the
 * temporary inlined. The panel shows its before/after diff. There are NO selection
 ...
 *
 * A banner surfaces the `decline` precondition the engine reports and the panel
 * refuses to apply on (the target is not an inlinable temporary). Inlining introduces
 * no shadowing, so there is no collision. Kept free of any `vscode` dependency so it
 * unit-tests directly.
 */
import { InlineTemporaryChange, InlineTemporaryOutOfScope } from './inlineTemporaryPreview';
...
function headerLabel(change: InlineTemporaryChange): string {
...
function renderCard(change: InlineTemporaryChange): string {
...
export function renderInlineTemporaryCards(changes: InlineTemporaryChange[]): string {
  return changes.map(renderCard).join('\n');
}

function renderBanner(oos: InlineTemporaryOutOfScope): string {
  const lines: string[] = [];
  if (oos.decline) {
...
export interface InlineTemporaryPanelHtmlOptions {
  name: string;
  total: number;
...
export function renderInlineTemporaryPanelHtml(opts: InlineTemporaryPanelHtmlOptions): string {
  const { name, total, changes, done, outOfScope, nonce, script } = opts;
  const cards = renderInlineTemporaryCards(changes);
...
<body data-total="${total}">
  <header>
    <div class="title">Inline temporary <code>${escapeHtml(name)}</code></div>
    <div class="actions">
  ...
  <div class="summary">
    Replaces each use of the temporary with its value and removes the declaration.
