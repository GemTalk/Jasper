/**
 * Pure HTML rendering for the split-class (V8 / extract class) preview panel. Every row is a CORE
 * change (the refactoring is all-or-nothing), rendered with a checked, DISABLED checkbox. A
 * `classAdd` shows the new component class's generated definition as an all-added block; a
 * `classDefinitionEdit` and a `methodAdd` show a before/after diff; a `classReparent` (a descendant
 * recompiled only to re-point at its freshly versioned ancestor) is a compact note. A precondition
 * failure sits in a banner; a note explains that existing instances are not migrated. Paginated
 * exactly like the other refactoring panels, reusing renameMethodPanelView.js for the DOM behaviour.
 *
 * Kept free of any `vscode` dependency so it unit-tests directly.
 */
import { SplitChange, SplitOutOfScope, splitChangeLabel } from './splitClassPreview';
...
function renderCard(change: SplitChange): string {
  const label = escapeHtml(splitChangeLabel(change));
  ...
  if (change.kind === 'classReparent') {
    ...
    // classAdd + methodAdd carry only newSource; a classDefinitionEdit carries both — render just
    // the side that's present so single-sided rows don't show a phantom empty line.
...
export function renderSplitCards(changes: SplitChange[]): string {
  return changes.map((c) => renderCard(c)).join('\n');
}

function renderBanner(oos: SplitOutOfScope): string {
  if (!oos.decline) return '';
  return `<div class="oos">⛔ ${escapeHtml(oos.decline)}</div>`;
}

function renderNote(oos: SplitOutOfScope): string {
  if (!oos.note) return '';
  return `<div class="note">ℹ️ ${escapeHtml(oos.note)}</div>`;
}

export interface SplitPanelHtmlOptions {
  heading: string;
  total: number;
  changes: SplitChange[];
  done: boolean;
...
export function renderSplitPanelHtml(opts: SplitPanelHtmlOptions): string {
  const { heading, total, changes, done, outOfScope, nonce, script } = opts;
  const cards = renderSplitCards(changes);
