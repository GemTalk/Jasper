/**
 * The Inline Temporary (M4) preview panel. Shows the single method before/after;
 * Apply is server-side (recompile the one method, no commit). M4 is method-local and
 ...
 *
 * Reuses the SHARED webview behaviour renameMethodPanelView.js.
 */
import * as vscode from 'vscode';
import * as crypto from 'crypto';
import { StartInlineTemporaryPreview, PreviewPage, ApplyResult } from './inlineTemporaryPreview';
import {
 ...
 *  cancelled/closed it. */
export function showInlineTemporaryPanel(
  name: string,
  start: StartInlineTemporaryPreview,
  handlers: InlineTemporaryPanelHandlers,
): Promise<ApplyResult | undefined> {
  const panel = vscode.window.createWebviewPanel(
    'gemstoneInlineTemporary',
    `Inline ${name}`,
  ...
  panel.webview.html = renderInlineTemporaryPanelHtml({
    name,
    total: start.total,
  ...
  return new Promise<ApplyResult | undefined>((resolve) => {
    ...
    const fetchOne = async (): Promise<boolean> => {
      ...
      void panel.webview.postMessage({
        ...
        html: renderInlineTemporaryCards(page.changes),
        done,
      });
      return done;
    };
