...
// A controllable webview panel so the paginated change-signature panel's message
...
import { StartPreview } from '../changeSignaturePreview';
...
describe('showChangeSignaturePanel', () => {
  it('applies with the deselected ids and resolves the apply result', async () => {
    ...
    const result = showChangeSignaturePanel('at:', 'at:put:', start, handlers);
  ...
  it('fetches the next page when asked for more', async () => {
    ...
    void showChangeSignaturePanel('at:', 'at:put:', start, handlers);
