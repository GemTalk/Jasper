...
// the M4-specific contract: there are NO selection checkboxes, so nothing is ever
...
function change(id: string): InlineTemporaryChange {
  return {
    id,
...
function mount(changes: InlineTemporaryChange[], total: number, done: boolean) {
  const full = renderInlineTemporaryPanelHtml({
    name: 't',
    total,
    changes,
    done,
...
describe('inline-temporary preview panel', () => {
  ...
  it('appends a freshly-fetched page and marks the pager done', () => {
    ...
    handle.appendChanges(renderInlineTemporaryCards([change('2')]), true);
