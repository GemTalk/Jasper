/**
 * Pure HTML rendering for the instance-variable structure (V2 / V3 / V5) preview panel.
 * Every row is a CORE change (the refactoring is all-or-nothing), rendered with a checked,
 * DISABLED checkbox. A `classDefinitionEdit` and a `methodRecompile` show a two-sided
 * before/after diff; a `classReparent` (the descendant is recompiled only to re-point at
 * the new parent chain, its definition unchanged) is shown as a compact note. A
 * precondition failure sits in a banner; a note explains that existing instances are not
 * migrated. Paginated exactly like the other refactoring panels, reusing
 * renameMethodPanelView.js for the DOM behaviour.
 *
 * Kept free of any `vscode` dependency so it unit-tests directly.
 */
import { IvarChange, IvarOutOfScope, ivarChangeLabel } from './instVarStructurePreview';
...
function renderCard(change: IvarChange): string {
  const label = escapeHtml(ivarChangeLabel(change));
...
export function renderIvarCards(changes: IvarChange[]): string {
  return changes.map((c) => renderCard(c)).join('\n');
}

function renderBanner(oos: IvarOutOfScope): string {
  if (!oos.decline) return '';
  return `<div class="oos">⛔ ${escapeHtml(oos.decline)}</div>`;
}

function renderNote(oos: IvarOutOfScope): string {
  if (!oos.note) return '';
  return `<div class="note">ℹ️ ${escapeHtml(oos.note)}</div>`;
}

export interface IvarPanelHtmlOptions {
  heading: string;
  total: number;
  changes: IvarChange[];
  done: boolean;
...
export function renderIvarPanelHtml(opts: IvarPanelHtmlOptions): string {
  const { heading, total, changes, done, outOfScope, nonce, script } = opts;
  const cards = renderIvarCards(changes);
...
<head>
  ...
  <title>Instance Variable Structure</title>
  <style>
    body {
