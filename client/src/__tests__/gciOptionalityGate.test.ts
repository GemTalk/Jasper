 ...
 * (absent from the Windows client library) and `removedIn` (gone from the next major release on).
 ...
 *
 * A test rather than a lint rule, but not because a rule couldn't read the
 * registry — it can. `eslint.config.mjs` reaches TypeScript through Node's
 * type-stripping (which is why `optionalFunctions.ts` names its generated half
 * with an explicit `.ts` specifier, and is exempted from the repo-wide ban on
 * those). So a selector-based rule would not have to restate these names, and
 * moving this check there is on the table; it lives here for now.
 */
...
describe('GCI optionality gate', () => {
  ...
  describe('hazardFor', () => {
    it.each([
      ...
      ['GciTsEncrypt', 'removed in nextMajor'],
      [
        'GciTsNbLogin_',
