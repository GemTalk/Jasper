...
describe('session row inline button order', () => {
  it('leads with the most-used safe actions and trails with Logout, without the rare backup actions', () => {
    ...
    // Session Configuration opens the session's configuration + live-maintenance page
    // (configuration is session-scoped, and Ping now lives inside it) — placed
    // just before the session-ending Logout, which stays last.
