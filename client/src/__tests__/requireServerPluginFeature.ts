...
import type { PluginFeatureRef } from '../serverPlugin/pluginFeatures';

/**
 * Gates keyed off the shared plugin-feature registry. Pass a feature straight from
 * the registry (`pluginFeatures.refactoring` from `serverPlugin/pluginFeatures.ts`)
 * at the top of an integration test to declare which runtime scenario the test
 * applies to — feature present or feature absent — and skip it, with a reason,
 * when the connected stone isn't in that scenario. This lets each CI pass (bare
 * stone vs. plugin-installed stone) run exactly the subset of tests that make
 * sense for it: inapplicable tests report as skipped rather than failing.
 *
...
/**
 * Present-world gate: call at the top of a test that only applies when `feature`
 * IS present, so the test exercises the feature's behavior. Skips when the stone
...
/**
 * Absent-world gate: call at the top of a test that only applies when `feature` is
