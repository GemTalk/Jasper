 ...
 */
function temporaryServerPath(session: unknown, gciLibrary: GciLibrary) {
 ...
 */
function createServerDirectory(folderPath: string, gciLibrary: GciLibrary, session: unknown) {
  gciLibrary.executeDiscardingResult(session, `GsFile createServerDirectory: '${folderPath}'`);
}
...
export function withTemporaryServerFolderDo(
  ...
  createServerDirectory(temporaryFolderPath, gciLibrary, session);
