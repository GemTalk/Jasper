...
export function serverBackupFilePaths(execute: QueryExecutor, folder: string): string[] {
  const code = `
    | backupPaths backupFiles filesByModifiedDesc result |
     backupPaths := (GsFile contentsOfDirectory: '${escapeString(folder)}' onClient: false)
       select: [ :path | path endsWith: '${GEMSTONE_BACKUP_EXTENSION}' ].
     backupFiles := backupPaths collect: [ :path | GsFile openReadOnServer: path ].
     filesByModifiedDesc := backupFiles sortWithBlock: [ :file :anotherFile | file lastModified > anotherFile lastModified ].

     result := WriteStream on: String new.
     filesByModifiedDesc do: [ :file |
       result nextPutAll: file pathName; lf.
       file close ].
