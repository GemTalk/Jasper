...
| Call any registry symbol from production code                | [`gciOptionalityGate.test.ts`](../../client/src/__tests__/gciOptionalityGate.test.ts) fails and names every call site, with the hazard the entry implies — absent before its `addedIn` floor, absent from the Windows client library, or removed in 4.0. The remedy is to put the conditional in `client/src/gciLibrary/` and call a helper from there; `ALLOWED_OPTIONAL_FUNCTION` is the legacy escape hatch, and its three entries exist to be removed | `npm test`, stone-free                                 |
