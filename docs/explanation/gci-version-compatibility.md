...
| Call any registry symbol from production code                | `eslint.config.mjs`'s `OPTIONAL_GCI_CALL` block fails and names the hazard the entry implies — absent before its `addedIn` floor, absent from the Windows client library, or removed in 4.0. Selectors are generated from the registry, so the gated set is never restated. The remedy is to put the conditional in `client/src/gciLibrary/` and call a helper from there; the fallback is an `eslint-disable-next-line` at the call site whose `-- reason` names the guard and the degraded path | `npm run lint`                                         |
...
Note where the load is carried: the _registry-vs-headers_ checks are stone-free unit tests that run on every `npm test`, the _don't-call-it-unguarded_ check is lint (so it also shows up in your editor, on the offending line, before you commit), and the _library-vs-reality_ checks need the CI matrix (five GemStone versions × two platforms, each run twice — bare and with the server plugin). All three are required CI gates.
