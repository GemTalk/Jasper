...
| Call any registry symbol from production code                             | [`gciOptionalityGate.test.ts`](../../client/src/__tests__/gciOptionalityGate.test.ts) names every call site with the hazard the entry implies — absent before its `addedIn` floor, absent from the Windows client library, or removed in 4.0 — unless you add it to `ALLOWED_OPTIONAL_FUNCTION`, the legacy escape hatch whose three entries exist to be removed | `npm test`, stone-free                            |
