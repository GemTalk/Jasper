...
**Current floor:** see `engines.vscode` / `engines.node` in the root `package.json` for the VS Code/Node floor, and the `typescript` devDependency range there for the compiler floor.
